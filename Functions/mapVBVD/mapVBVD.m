...
arg.joinPatRefAndIma =false; % ATH
...
while k <= numel(varargin)
    ...
    switch lower(varargin{k})
        ...
        case {'joinpatrefandima'}
            if numel(varargin) > k && ~ischar(varargin{k+1})
                arg.joinPatRefAndIma = logical(varargin{k+1});
                k = k+2;
            else
                arg.joinPatRefAndIma = true;
                k = k+1;
            end
        otherwise
...
for s=1:NScans
    ...
    if arg.bReadImaScan
        clear tmpMdh
        if(arg.joinPatRefAndIma)
            isCurrScan = logical( mask.MDH_IMASCAN | mask.MDH_PATREFSCAN);
        else
...
mask.MDH_VOP                = min(bitand(mdh.aulEvalInfoMask(:,2), 2^(53-32)),1); % was 0 in VD  ATH adapted to :,2
