...
%
% Modified by Aaron Hess (aaron.hess@cardiov.ox.ac.uk)
% Added function readVOP to read RF pulses that span multiple ADC events
% and RF pulses of different lengths
% the function returns a cell object the lenght of the number of differnt
% RF pulse types, and a second cell object to specify which index each is 
%
...
properties(GetAccess='public', SetAccess='public')
    ...
    % ATH Add readout shift correction factor
    readout_shift_mm_per_fov
end
...
properties(GetAccess='public', SetAccess='protected')
    ...
    nCol  % mdh info for VOP ATH added
...
methods
    ...
    function this = twix_map_obj(arg,dataType,fname,version,rstraj)
        ...
        % ATH a readoutshift factor
        this.readout_shift_mm_per_fov = [];
...
%         end
    ...
    function this = clean(this)
        ...
        % ATH upate for handleing vop data
        if(strcmp(this.dataType,'vop'))
            this.nCol = this.NCol;
        end
    ...
    function varargout = subsref(this, S)
        ...
        % ATH 09/2017 record for return parameter
        ixToTarg_out = ixToTarg;  % send as output for mapping of other mdh parameters
        ixToRaw_out = ixToRaw;
        ...
        if(nargout>2)
            varargout{2} = ixToTarg_out;
            varargout{3} = ixToRaw_out;
        end
    end % of subsref
    ...
    function out = readData(this,mem,cIxToTarg,cIxToRaw,selRange,selRangeSz,outSize)
        ...
        bApplyROShift = ~isempty(this.readout_shift_mm_per_fov);
        if(bApplyROShift)
            ro_shift = this.readout_shift_mm_per_fov(cIxToRaw);
            lis_cor = repmat((1:this.NCol)' - this.centerCol(1),[1 this.NCha])/2; % repeat for channel
        end
        ...
        for k = 1:kMax
            ...
            if blockCtr == blockSz || k == kMax || (isBrokenRead && blockCtr > 1)
                ...
                % ATH
                if bApplyROShift                    
                    block = block.*exp(bsxfun(@times,lis_cor,reshape(-1i*2*pi*ro_shift(ix),[1 1 numel(ix)])));                    
                end
    ...
    function varargout = readVOP(this)
        % method to read vop data
        % handle rf pulses from multiple adc events and different lengths
        vop_length_list = [];
        ...
        % Scan through and find how many of each each type of pulse
        % there are and their size
        pulse_len = 0;
        this_pulse = [];
        pulse_counter = 0;
        nList = 0;
        ...
        % do this twice, 
        % (1) to workout memory requirments
        % (2) read in the data
        for i_memprep = 1:2
            if(i_memprep==2)
                for list_ind = 1:nList
                    out{list_ind} = single(zeros(vop_length_list(list_ind),this.NCha,cnt(list_ind)));
                    inx{list_ind} = single(zeros(1,cnt(list_ind)));
                end
                cnt(:) = 0; % reset counter
            end
            for k=1:numel(mem)
                if(i_memprep==2)
                    nByte = this.NCha*(this.freadInfo.szChannelHeader+8*this.nCol(k));    
                    % size for fread
                    readSize    = [2 nByte/8];
                    % reshape size
                    readShape = [this.nCol(k)+this.freadInfo.szChannelHeader/8, this.NCha];
                    % we need to cut MDHs from fread data
                    readCut   = this.freadInfo.szChannelHeader/8+1 ...
                                      : this.nCol(k)+this.freadInfo.szChannelHeader/8;
                    ...
                    this_pulse = [this_pulse;raw];
                end
                % freeParam1 = number of blocks
                % vopDataIde = counter for this block
                pulse_len = pulse_len+this.nCol(k);
                if (this.Ide(k)==this.freeParam(1,k)) % all blocks read in

                    [vop_length_list, list_ind] = this.UpdateList(vop_length_list, pulse_len);
                    if (list_ind>nList ) %add a list                            
                        nList = list_ind;
                        cnt(list_ind) = 0;
                        
                    end
                    cnt(list_ind) = cnt(list_ind) +1;
%                     pulse_counter=pulse_counter+1;
                    if(i_memprep==2)
                        out{list_ind}(:,:,cnt(list_ind))=this_pulse;
                        inx{list_ind}(cnt(list_ind)) = k; %which memory element
                    end
...
%             out{k} = reshape(out{k},[size(out{k},1),this.NCha,cnt(k)]);
%             inx{k} = cell2mat(inx{k});
%         end
    ...
    % ATH allow a readout motion shift by applying a phase role
    % this should be in units of mm divided by the RO_FOV, and scaled by
    % the correction factor needed
    function set.readout_shift_mm_per_fov(this,val)
    
        if(numel(val)==numel(this.memPos))
            this.readout_shift_mm_per_fov = val;
        else
            warning('requested shift vector does not equal file length')
        end
    end
    function setDefaultFlags(this)
        ...
        this.arg.joinPatRefAndIma    = true;
    ...
    function out = get.flagIgnoreSeg(this)
        ix = ismember(this.dataDims, 'Seg');
        out = this.flagAverageDim(ix);
%         this.fullSize(ix) = 1; % ATH Added but does not work
    end
    ...
    % A Hess added for retro gating
    % update the phase bin according to ECG time
    function out = DoCardiacRetoBinning(this,NBins)
        old_phs = this.Phs(:);
        d_pmu = diff(this.pmutime);
        nRR = nnz(d_pmu<0);
        lin_perRR = length(this.pmutime)/nRR;
        lin_perBin = lin_perRR/NBins;
        
        pmu_tick_per_lin = mean(d_pmu(d_pmu>0));
        this.Phs = round(this.pmutime/pmu_tick_per_lin/lin_perBin)+1;
        this.Phs(this.Phs>NBins) = NBins;
        this.NPhs = NBins;
        out = 0;
    end
    function out = DoCardiacNewPhase(this,newPhs)
        if(length(newPhs)>this.NAcq)
            newPhs(this.NAcq+1:end ) = [];
        end
        this.Phs(1:length(newPhs)) = newPhs(:);
        this.NPhs = max(newPhs)+1;
        this.Phs(length(newPhs)+1:end) = this.NPhs;
        
        ix = ismember(this.dataDims, 'Phs');
        this.fullSize(ix) = this.NPhs;
       out = 0; 
    end
    
    
    function out = DoRespiratoryRetroBinning(this,newSet)
        this.Set(:) = newSet(:);
        this.NSet = max(newSet);
        ix = ismember(this.dataDims, 'Set');
        this.fullSize(ix) = this.NSet;
       out = 0; 
    end
    ...
    function [selRange,selRangeSz,outSize] = calcRange(this,S)
        ...
        if ( isempty(S.subs) || strcmpi(S.subs(1),'') )
            ...
            for k=1:numel(selRange)
                %if max(selRange{k}) > this.dataSize(k) ATH AVEDIM changed this
                if max(selRange{k}) > this.fullSize(k)
        ...
        % ATH AVEDIM, may want to remove below and replace with outSize dont want to do this for now
        % outSize(this.flagAverageDim) = 1;
...
%        ixToRaw(ixToTarget(end:-1:1)) = numel(ixToTarget):-1:1; % ATH do this backwards to keep the first
    end
end
...
methods (Static)
    ...
    function [out_list, list_ind] = UpdateList(in_list, var)
        % ATH added 2017 for VOP functions
